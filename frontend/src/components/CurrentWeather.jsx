...
export default function CurrentWeather({data}) {
    ...
    const codeHint = (humidity && humidity > 85) ? "rain" : (temp_c < 5 ? "snow" : "clear");
    return (
        <div className="card p-6 flex items-center justify-between gap-8">
            <div>
                <h2 className="text-2xl font-bold text-slate-800">{city}</h2>
                <p className="text-slate-500">{dayjs(time_utc).format("dddd, MMM D • HH:mm")}</p>
                <div className="mt-4 flex items-center gap-8">
                    <div className="text-6xl font-extrabold text-slate-800">{Math.round(temp_c)}°C</div>
